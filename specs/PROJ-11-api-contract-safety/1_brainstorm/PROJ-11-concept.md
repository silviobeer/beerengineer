...
- Public route additions, changes, and removals update both `openapi.json` and `docs/api-contract.md`. The prose update rule is concrete: the canonical `METHOD /path` string (see canonical form below) must appear in `docs/api-contract.md` either at the start of a list item or inside a fenced code block — substring-anywhere is not sufficient and would let incidental sentence mentions satisfy the rule. Routes without that presence must carry a per-route exception with reason.
- All three sources — engine route declarations, `openapi.json` paths, and `docs/api-contract.md` prose mentions — are normalized to a single canonical `METHOD /path` form before comparison. The mapping table is pinned by requirements; the `:key` style currently used in `docs/api-contract.md` and the OpenAPI `{key}` style must collapse to the same canonical string, otherwise the prose-presence rule fails on existing real routes the day the gate ships.
- Methods are uppercased before comparison. `HEAD` and `OPTIONS` are not part of the parity surface (treated as automatic exceptions) since they are protocol-level and not enumerated in the prose contract.
...
- The parity input is a single committed source of route truth. Preferred order: (a) a programmatic registry exported by PROJ-10's route ownership shape; (b) AST scan of `apps/engine/src/api/routes/*` if no registry exists; (c) runtime introspection by booting the engine in a test harness only if neither (a) nor (b) is workable, since runtime introspection is heavier and more fragile in CI than static sources. Brittle text scraping of `server.ts` is not allowed. If PROJ-10 does not export a registry, PROJ-11 will add one as its first wave rather than block on a PROJ-10 amendment. **Post-PROJ-10 status**: `apps/engine/src/api/routeRegistration.ts` exports only `registerApiRoutes` — no programmatic registry. Wave 1 of PROJ-11 must therefore add a declarative route registry (option b → promoted to a typed export in `routeRegistration.ts`) as its first deliverable.
- Engine route declarations are the source of truth when the gate reports drift. Fixes flow from engine → OpenAPI → prose, never the reverse: a stale OpenAPI entry must not be "fixed" by adding a fake handler to match it.
- The exception list is a single committed file (suggested location `apps/engine/src/api/route-parity-exceptions.json`; final location and schema pinned by requirements) with fields `{ method, path, reason, category }` per entry. The `category` field is a label for grouping and reporting only — it is never used as a wildcard match key. Entries are reviewed in PR diff like any other code change.
...
- Dynamic route syntax differences normalize to a single canonical template form applied uniformly to engine declarations, OpenAPI paths, and prose mentions. The committed rule: regex captures and engine path-parameter syntax map to `{paramN}` (or named `{name}` when the engine declaration provides one); OpenAPI `{name}` parameters and prose `:name` parameters collapse to the same normalized string. Requirements pin the exact mapping table.
...
- Engineering rule alignment: the parity gate module must have a real production caller (the standard test command, since the gate is required to run there), satisfying the "no test-only engine modules" rule in the repo CLAUDE.md.
...
- For requirements-engineer: must pin (1) the single parity input source — registry export preferred, AST scan fallback, runtime introspection only as last resort; (2) the exact path normalization mapping table, applied identically to engine declarations, OpenAPI paths, and prose mentions; (3) the prose update rule as canonical `METHOD /path` presence in `docs/api-contract.md` at the start of a list item or inside a fenced code block (substring-anywhere is rejected); (4) the exception file location (suggested `apps/engine/src/api/route-parity-exceptions.json`) and `{ method, path, reason, category }` schema, with `category` as label-only; (5) the exact test command the gate hooks into (named, not "the standard test command"); (6) explicit rename failure messaging; (7) the permanent `/openapi.json` exception entry; (8) method casing normalization and `HEAD`/`OPTIONS` automatic-exception treatment; (9) engine declarations as source of truth on drift. Also specify parity outputs, failure behavior, and the documented limits of method/path-only checking.
