...
The context has three conceptual groups:

- Local Git context: workspace root, repository readiness, branch/worktree
  context, and diff/base references needed by workspace and review flows.
- GitHub provider context: remote URL, owner, repo, default branch, and whether
  the current flow has enough GitHub context for provider actions.
- GitHub tool context: `gh` availability and authentication readiness for flows
  that need `gh` actions.

Optional capabilities may request this context from orchestration, but they do
not re-derive it.
...
| Field | Contract |
|---|---|
| Capability identity | Required stable capability ID. |
| Lifecycle or phase | Required review lifecycle marker. |
| Outcome | Required closed-set outcome. |
| Blocking intent | Required boolean indicating whether this outcome blocks the current flow. |
| Summary | Required human-readable one-line explanation. |
| Reason | Required for every non-`ran` outcome. |
| Artifacts | Present as a list; empty when no artifact exists. |
| Tool result | Present only when the capability has tool-specific data to preserve. |

The closed review outcome set is exactly:

- `ran`: the capability completed and produced a meaningful tool-specific
  result.
- `skipped`: the capability was intentionally not attempted because the flow or
  policy said not to run it.
- `not_configured`: required local configuration, credentials, CLI setup, or
  project metadata is absent.
- `failed`: the capability was attempted and hit an execution or service
  failure.
- `not_meaningful`: the capability could be reached, but the available input or
  produced artifacts cannot support a meaningful assessment for this run.
...
### 7. Define capability CLI exit-code categories once

Capability CLI commands use one exit-code family:

| Code | Meaning |
|---|---|
| `0` | Command succeeded without required capability failure. |
| `20` | Usage, argument, or workspace-selection error. |
| `30` | Transport or API communication error. |
| `40` | Required capability failure. |
| `41` | Optional capability warning, skipped state, or not-meaningful state when the command's purpose is to surface that warning state. |

Why: capability commands need predictable automation behavior. Optional
Sonar/CodeRabbit warning states must not look like required Git failures.

Affects: PROJ-3-PRD-1, PRD-3, PRD-5.

### 8. Make Sonar lifecycle explicit and conservative
...
Sonar enablement uses a best-effort write strategy with audit and re-enable
recovery. If an optional Sonar write fails during `workspace add --sonar`, the
workspace registration still succeeds when required workspace preconditions
passed, and the Sonar result is recorded as `failed` with a reason. Missing
Sonar prerequisites are recorded as `not_configured`. Sonar audit is
responsible for detecting partial Sonar-owned states that can be recovered by
enablement or repair.
...
### 9. Preserve API and UI compatibility as a constraint

Existing setup, settings, workspace, and review UI flows must keep working.
The compatibility sources are `apps/engine/src/api/openapi.json`,
`docs/api-contract.md`, and existing UI API consumers. These contracts are
frozen by default for this PROJ. A contract-breaking change requires an
explicit architecture or wave-plan decision and must update the API contract
and affected existing UI consumers in the same wave.
...
### 10. Keep update-mode separate but align readiness terms

Update-mode remains a beerengineer self-update flow. It shares GitHub and
Sonar readiness terminology and helper behavior where meanings overlap. If
update-mode has different inputs and cannot use the same helper, it still uses
the same readiness meaning and documents why the helper is not shared. It does
not call workspace capability orchestration.
...
### 11. Migrate incrementally, not as one cutover

Implementation starts with shared capability terms and tests, then move
...
The ordering above describes migration groups, not PRD file order. Wave plans
may split a PRD across multiple waves when that is the safer migration path.

### 12. Keep CodeRabbit intentionally lighter than Sonar

CodeRabbit is a review and readiness capability, not a quality-scope lifecycle
capability in PROJ-3. It does not receive audit/repair unless architecture or
implementation discovers a real CodeRabbit drift surface.

Why: Sonar owns project quality-scope configuration that drifts with source,
test, and coverage layout. CodeRabbit's initial surface is CLI readiness,
configuration presence, and review execution. Giving CodeRabbit a fake
lifecycle would add symmetry without product value.

Affects: PROJ-3-PRD-1, PRD-2, PRD-4, PRD-5.
...
PROJ-3 does not pin or install external tool versions. If version requirements
for `gh`, Sonar scanner, or CodeRabbit CLI are needed, they belong in setup or
capability readiness checks, not package dependencies.
