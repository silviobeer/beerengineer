...
async function runProject(initialCtx: ProjectContext): Promise<void> {
  let ctx = initialCtx
  ctx = { ...ctx, prd: await requirements(ctx) }
  ctx = { ...ctx, architecture: await architecture(assertWithPrd(ctx)) }
  ctx = { ...ctx, plan: await planning(assertWithArchitecture(ctx)) }
  ctx = { ...ctx, executionSummaries: await execution(assertWithPlan(ctx)) }
  ctx = { ...ctx, projectReview: await projectReview(assertWithExecution(ctx)) }
  await qa(ctx)
  ctx = { ...ctx, documentation: await documentation(assertWithProjectReview(ctx)) }
  await handoffCandidate(assertWithDocumentation(ctx))
}
