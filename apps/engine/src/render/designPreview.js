...
function validateDesignMetadata(artifact) {
    if (typeof artifact.tone !== "string" || artifact.tone.length === 0) {
        throw new TypeError("Invalid design artifact from LLM: artifact.tone is missing or not a string. " +
            "The LLM response may have been truncated or returned a malformed structure.");
    }
    if (!Array.isArray(artifact.antiPatterns)) {
        throw new TypeError("Invalid design artifact from LLM: artifact.antiPatterns is not an array. " +
            "Every design artifact must include an antiPatterns array — retry or inspect the LLM output.");
    }
}
function validateDesignTokens(artifact) {
    if (!artifact.tokens || typeof artifact.tokens !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.tokens is missing. " +
    ...
    if (!artifact.tokens.light || typeof artifact.tokens.light !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.tokens.light is missing. " +
            "Every design artifact must include a light-mode token palette — retry or inspect the LLM output.");
    }
}
function validateDesignTypography(artifact) {
    if (!artifact.typography || typeof artifact.typography !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.typography is missing. " +
    ...
    if (!artifact.typography.scale || typeof artifact.typography.scale !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.typography.scale is missing or not an object. " +
...
function validateFontRole(role, roleName) {
    if (!role || typeof role !== "object") {
        throw new Error(`Invalid design artifact from LLM: artifact.typography.${roleName} is missing. ` +
            `Every design artifact must include typography.${roleName} with family and weight — retry or inspect the LLM output.`);
    }
    requireNonEmptyString(role.family, `Invalid design artifact from LLM: artifact.typography.${roleName}.family is missing or not a string. ` +
        `Every design artifact must have a non-empty ${roleName} font family — retry or inspect the LLM output.`);
    requireNonEmptyString(role.weight, `Invalid design artifact from LLM: artifact.typography.${roleName}.weight is missing or not a string. ` +
        `Every design artifact must have a non-empty ${roleName} font weight — retry or inspect the LLM output.`);
}
function validateDesignSpacing(artifact) {
    if (!artifact.spacing || typeof artifact.spacing !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.spacing is missing. " +
            "Every design artifact must include spacing tokens — retry or inspect the LLM output.");
    }
    for (const field of ["baseUnit", "sectionPadding", "cardPadding", "contentMaxWidth"]) {
        requireNonEmptyString(artifact.spacing[field], `Invalid design artifact from LLM: artifact.spacing.${field} is missing or not a string. ` +
            "Every design artifact must have all spacing token fields — retry or inspect the LLM output.");
    }
}
function validateDesignBorders(artifact) {
    if (!artifact.borders || typeof artifact.borders !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.borders is missing. " +
            "Every design artifact must include borders tokens — retry or inspect the LLM output.");
    }
    for (const field of ["buttons", "cards", "badges"]) {
        requireNonEmptyString(artifact.borders[field], `Invalid design artifact from LLM: artifact.borders.${field} is missing or not a string. ` +
            "Every design artifact must have all border token fields — retry or inspect the LLM output.");
    }
}
function validateDesignShadows(artifact) {
    if (!artifact.shadows || typeof artifact.shadows !== "object") {
        throw new Error("Invalid design artifact from LLM: artifact.shadows is missing or not an object. " +
            "Every design artifact must include a shadows Record<string, string> — retry or inspect the LLM output.");
    }
}
function validateMockupHtmlPerScreen(mockupHtmlPerScreen) {
    if (mockupHtmlPerScreen === undefined)
        return;
    if (typeof mockupHtmlPerScreen !== "object" || Array.isArray(mockupHtmlPerScreen)) {
        throw new TypeError("Invalid design artifact from LLM: artifact.mockupHtmlPerScreen is present but not an object. " +
    ...
    for (const [screenId, html] of Object.entries(mockupHtmlPerScreen)) {
        requireNonEmptyString(html, `Invalid design artifact from LLM: mockupHtmlPerScreen["${screenId}"] is empty or not a string. ` +
            "Each entry must be a non-empty HTML document — retry or inspect the LLM output.");
        const trimmed = html.trimStart().toLowerCase();
        if (!trimmed.startsWith("<!doctype") && !trimmed.startsWith("<html")) {
            throw new Error(`Invalid design artifact from LLM: mockupHtmlPerScreen["${screenId}"] does not start with <!doctype or <html. ` +
...
function safeCssValue(raw, pattern, fallback) {
    if (typeof raw !== "string")
        return fallback;
    const trimmed = raw.trim();
    return pattern.test(trimmed) ? trimmed : fallback;
}
function renderPalette(title, palette) {
...
export function renderDesignPreview(artifact) {
