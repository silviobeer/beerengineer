 ...
 */
export function renderMockupFile(screenId, design) {
    if (!design.mockupHtmlPerScreen) {
        throw new Error(`renderMockupFile: design artifact is missing mockupHtmlPerScreen. ` +
    ...
    if (typeof html !== "string" || html.trim().length === 0) {
        throw new Error(`renderMockupFile: no HTML found for screenId="${screenId}" in mockupHtmlPerScreen. ` +
            `Available screens: ${Object.keys(design.mockupHtmlPerScreen).join(", ") || "(none)"}.`);
    }
    const trimmed = html.trimStart().toLowerCase();
    if (!trimmed.startsWith("<!doctype") && !trimmed.startsWith("<html")) {
        throw new Error(`renderMockupFile: mockupHtmlPerScreen["${screenId}"] does not start with <!doctype or <html — not a valid HTML document.`);
    }
    return html;
}
 ...
 */
export function renderMockupSitemap(screens, runId, publicBaseUrl) {
