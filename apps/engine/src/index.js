...
import { resolveItemReference, runDoctor, withRepos } from "./cli/common.js";
import { resolveUiLaunchUrl, resolveUiWorkspacePath, startEngine, startUi } from "./cli/ui.js";
import { runChatAnswerCommand, runChatListCommand, runChatSendCommand, runItemDesignCommand, runItemGetCommand, runItemOpenCommand, runItemPreviewCommand, runItemsCommand, runRunGetCommand, runRunListCommand, runRunMessagesCommand, runRunOpenCommand, runRunTailCommand, runRunWatchCommand, runStatusCommand, runWorkspaceItemsCommand, runItemWireframesCommand, } from "./cli/commands/overview.js";
import { runWorkspaceAddCommand, runWorkspaceBackfillCommand, runWorkspaceGetCommand, runWorkspaceListCommand, runWorkspaceOpenCommand, runWorkspacePreviewCommand, runWorkspaceRemoveCommand, runWorkspaceUseCommand, runWorkspaceWorktreeGcCommand, } from "./cli/commands/workspaces.js";
...
import { loadEffectiveConfig } from "./cli/common.js";
export { parseArgs, resolveItemReference, resolveUiLaunchUrl, resolveUiWorkspacePath, runDoctor, startEngine, startUi, runItemAction };
async function runNotificationsTestCommand(channel) {
...
export async function main(argv = process.argv.slice(2)) {
    ...
    if (cmd.kind === "workflow") {
        try {
            ...
            if (cmd.json) {
                process.stderr.write(`${JSON.stringify({ type: "cli_error", message: err.message })}\n`);
            }
            else {
                console.error("\n  FEHLER:", err.message);
            }
    ...
    process.exit(await handler(cmd));
}
