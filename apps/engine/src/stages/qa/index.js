...
import { createQaReview, createQaStage } from "../../llm/registry.js";
import { renderPrdDigest } from "../../render/artifactDigests.js";
import { ask } from "../../sim/human.js";
export async function qa(ctx, llm) {
    ...
    if (!ctx.prd)
        throw new Error("qa stage invariant: ctx.prd is required");
    if (!ctx.projectReview)
        throw new Error("qa stage invariant: ctx.projectReview is required");
    await runStage({
        stageId: "qa",
        ...
        async persistArtifacts(_run, artifact) {
            ...
            return [
                ...
                summaryArtifactFile("qa", stageSummary(_run, [
