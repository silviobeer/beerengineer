...
export async function mergeGate(context, git, blockRun) {
    ...
    if (answer === "cancel") {
        ...
        await blockRun(context, `Operator postponed merge of ${itemBranch} into ${git.mode.baseBranch}`, {
    ...
    if (answer !== "promote") {
        await blockRun(context, `Merge gate received unsupported answer for ${itemBranch}: ${answer || "<empty>"}`, {
    ...
    try {
        ...
        await blockRun(context, `Merge into ${git.mode.baseBranch} failed for ${itemBranch}: ${error.message}`, {
            cause: "merge_gate_failed",
            scope: { type: "stage", runId: activeRun.runId, stageId: "merge-gate" },
            branch: itemBranch,
        });
