...
import { createProjectReviewReview, createProjectReviewStage } from "../../llm/registry.js";
...
function findingsBySeverity(artifact) {
...
function sanitizeExecutionSummaries(executionSummaries) {
...
function isTextReviewTarget(path) {
    return /\.(md|txt|html?|css|js|jsx|ts|tsx|json|ya?ml|sh)$/i.test(path);
}
function reviewTargetPaths(executionSummaries) {
...
function readBranchFile(workspaceRoot, branch, path) {
    const result = runGit(workspaceRoot, ["show", `${branch}:${path}`]);
    return result.ok ? result.stdout : null;
}
export function collectProjectReviewRepoEvidence(ctx) {
...
export async function projectReview(ctx, llm) {
    ...
    const { result } = await runStage({
        ...
        async persistArtifacts(run, artifact) {
            return [
                ...
                summaryArtifactFile("project-review", stageSummary(run, [
        ...
        async onApproved(artifact, run) {
            ...
            if (artifact.findings.length > 0) {
                stagePresent.dim(`Residual findings by severity: critical=${severityCounts.critical ?? 0}, high=${severityCounts.high ?? 0}, medium=${severityCounts.medium ?? 0}, low=${severityCounts.low ?? 0}`);
            }
