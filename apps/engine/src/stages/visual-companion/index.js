...
import { createVisualCompanionReview, createVisualCompanionStage } from "../../llm/registry.js";
import { renderWireframeFiles } from "../../render/wireframes.js";
import { ask } from "../../sim/human.js";
function buildScreenSummary(artifact) {
...
export async function visualCompanion(context, input, llm, codebase) {
    ...
    return runStageWithUserReview({
        ...
        async persistArtifacts(run, artifact) {
            ...
            writeFileSync(join(run.stageArtifactsDir, "wireframes.json"), JSON.stringify(enrichedArtifact, null, 2));
            const jsonFiles = [
                {
                    kind: "json",
            ...
            return [
                ...
                summaryArtifactFile("visual-companion", stageSummary(run, [`Screens: ${artifact.screens.length}`, `Mode: ${artifact.inputMode}`])),
            ];
        },
        buildGatePrompt: ({ artifact }) => {
            ...
            return (`Wireframe summary (${artifact.screens.length} ${screenLabel}):\n` +
                `${summary}\n\n` +
