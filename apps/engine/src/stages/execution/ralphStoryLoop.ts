...
import type { StoryCheckResult, StoryImplementationArtifact, StoryReviewArtifact } from "../../types.js"
import { buildReviewArtifact, printReviewResult, runStoryReview } from "./ralphStoryReview.js"
import {
  appendLog,
  consumePendingRemediation,
  cycleReviewPath,
  ensureBranchAndStartLog,
  logEntry,
  recordStoryBlocked,
  requireStoryBranch,
  writeJson,
...
async function runCoderCycleUntilGreen(
  ctx: RalphLoopContext,
  implementation: StoryImplementationArtifact,
  opts: { reviewCycle: number; feedback: string | undefined },
...
async function runOneIteration(
  ctx: RalphLoopContext,
  implementation: StoryImplementationArtifact,
  opts: { reviewCycle: number; iterationsThisCycle: number; feedback: string | undefined },
  ...
  implementation.priorAttempts = priorAttempts
...
async function executeLlmIteration(
  ctx: RalphLoopContext,
  implementation: StoryImplementationArtifact,
  opts: { iterationNumber: number; reviewCycle: number; isRemediation: boolean; feedback: string | undefined },
): Promise<{ summary: string; changedFiles: string[]; notes: string[] }> {
  if (!ctx.llm) throw new Error("LLM iteration requested without execution LLM configuration")
...
function commitIterationWorktree(
  ...
  if (!ctx.storyContext.worktreeRoot) return
  const sha = commitAll(
    ctx.storyContext.worktreeRoot,
    isRemediation
  ...
  if (sha) stagePresent.dim(`    → committed ${ctx.storyContext.story.id} iteration ${iterationNumber}: ${sha.slice(0, 8)}`)
}
...
async function runOneReviewCycle(
  ...
  await appendLog(ctx.paths.logPath, logEntry(
