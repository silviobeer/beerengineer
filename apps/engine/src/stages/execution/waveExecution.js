...
import { runRalphStory, writeWaveSummary } from "./ralphRuntime.js";
...
export function parallelStoriesFlagEnabled() {
...
export async function execution(ctx, resume, llm, git = createGitAdapter(ctx)) {
    ...
    const summaries = [];
    const completedWaveIds = new Set();
    for (const wave of orderedWaves) {
        if (resume?.waveNumber && wave.number < resume.waveNumber) {
            const persisted = await readJsonIfExists(layout.waveSummaryFile(ctx, wave.number));
            if (!persisted)
        ...
        completedWaveIds.add(wave.id);
        if (resume?.waveNumber === wave.number)
            resume = undefined;
    }
    stagePresent.ok("All waves complete\n");
    return summaries;
}
export function assertWaveSucceeded(wave, summary) {
    if (summary.storiesBlocked.length === 0)
        return;
    throw new Error(`Wave ${wave.id} blocked stories: ${summary.storiesBlocked.join(", ")}.`);
}
function assertWaveDependenciesSatisfied(wave, completedWaveIds) {
...
function expectedSharedFilesForWave(wave) {
...
async function executeWave(ctx, wave, storyById, screenOwners, git, resume, llm) {
    ...
    let waveBranchOpQueue = Promise.resolve();
    const enqueueWaveBranchOp = (op) => {
    ...
    const run = async (story) => {
        ...
        let storyWorktreeRoot;
        let result;
        try {
            storyWorktreeRoot =
                git.ensureStoryWorktree(ctx.project.id, wave.number, resolved.id, layout.executionStoryWorktreeDir(ctx, wave.number, resolved.id)) ?? undefined;
            ...
            if (result.implementation.status === "passed") {
                await enqueueWaveBranchOp(() => git.mergeStoryIntoWave(ctx.project.id, wave.number, resolved.id, {
            ...
            if (storyWorktreeRoot) {
                try {
                    ...
                    stagePresent.dim(`worktree cleanup failed for ${storyWorktreeRoot}: ${err.message}`);
                }
            }
        }
    };
    ...
    if (summary.storiesBlocked.length > 0)
        await recordBlockedWave(ctx, wave, summary);
...
async function recordBlockedWave(ctx, wave, summary) {
    ...
    if (!active) {
        stagePresent.dim(`run_blocked emitted without active run context for runId=${ctx.runId} wave=${wave.number}; event will appear as unknown-item`);
    }
    emitEvent({
        type: "run_blocked",
        runId: ctx.runId,
...
function blockedPlaceholder(story, reason) {
    return {
...
async function sequentially(items, fn) {
    const out = [];
    for (const item of items)
        out.push(await fn(item));
    return out;
}
async function runStoriesInParallel(stories, run) {
...
function resolveStory(ref, storyById) {
...
async function implementStory(ctx, wave, story, screenOwners, opts = {}, llm) {
    ...
    const testPlan = opts.rerunTestWriter
        ...
        : (await readJsonIfExists(testPlanPath)) ?? await writeStoryTestPlan(ctx, wave, story, storyStageLlm);
    ...
    const result = await runRalphStory(storyContext, ctx, executionLlm);
