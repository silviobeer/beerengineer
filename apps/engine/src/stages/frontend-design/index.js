...
import { createFrontendDesignReview, createFrontendDesignStage } from "../../llm/registry.js";
...
function buildDesignSummary(artifact) {
...
function loadWireframesForRun(context, input) {
    ...
    if (!existsSync(vcPath))
        return undefined;
    try {
        return JSON.parse(readFileSync(vcPath, "utf8"));
    }
    catch {
 ...
 */
function resolvePublicBase(_context) {
    const activeRun = getActiveRun();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const configured = activeRun?.publicBaseUrl;
...
export async function frontendDesign(context, input, llm, codebase) {
    ...
    return runStageWithUserReview({
        ...
        async persistArtifacts(run, artifact) {
            ...
            writeFileSync(join(run.stageArtifactsDir, "design.json"), JSON.stringify(enrichedArtifact, null, 2));
            const coreArtifacts = [
                {
                    kind: "json",
                    label: "Design JSON",
            ...
            if (hasWireframes && wireframes) {
                ...
                for (const screen of wireframes.screens) {
                    try {
                        ...
                        stagePresent.warn(`frontend-design: could not write mockup for screen "${screen.id}": ${err.message}`);
                    }
                }
            ...
            return [
                ...coreArtifacts,
                ...mockupArtifacts,
                summaryArtifactFile("frontend-design", stageSummary(run, [
                    `Tone: ${artifact.tone}`,
        ...
        buildGatePrompt: ({ artifact, run }) => {
            ...
            const mockupFiles = run.files.filter(f => f.path.includes("/mockups/") && f.path.endsWith(".html") && !f.path.endsWith("sitemap.html"));
            let mockupSection = "";
