...
function topLevelRouteHandlers(context: RouteContext): Partial<Record<string, () => void | Promise<void>>> {
  return {
    ...
    "POST /setup/telegram/webhook": () => handleSetupTelegramWebhook(repos, context.appConfig, context.url, context.res),
    "POST /setup/telegram/verification": () => handleSetupTelegramVerification(repos, context.appConfig, context.url, context.res),
