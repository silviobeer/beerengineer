...
const orderedColumns = ["idea", "brainstorm", "frontend", "requirements", "implementation", "merge", "done"];
const columnTitles = {
    idea: "Idea",
...
function itemWorktreePath(rootPath, workspaceFsId, itemTitle, itemId) {
    if (!rootPath || !workspaceFsId)
        return null;
...
function reviewGateWaiting(actionsJson) {
    if (!actionsJson)
        return false;
    try {
...
function boardCardMeta(workspaceRoot, latestRun, openPrompt, item) {
...
export function getBoard(db, workspaceKey) {
    const workspace = workspaceKey
        ? db.prepare("SELECT * FROM workspaces WHERE key = ?").get(workspaceKey)
        : db.prepare("SELECT * FROM workspaces ORDER BY created_at ASC LIMIT 1").get();
    if (!workspace) {
        return {
    ...
    const items = db
        .prepare(`SELECT id, workspace_id, code, title, description, current_column, phase_status, current_stage
       FROM items WHERE workspace_id = ? ORDER BY created_at ASC`)
        .all(workspace.id);
    const projectCounts = new Map();
    const projectRows = db
        .prepare(`SELECT item_id, COUNT(*) as count FROM projects
       WHERE item_id IN (SELECT id FROM items WHERE workspace_id = ?)
       GROUP BY item_id`)
        .all(workspace.id);
    for (const row of projectRows)
        projectCounts.set(row.item_id, row.count);
    const latestRuns = new Map();
    const runRows = db
        .prepare(`SELECT id, item_id, status, recovery_status, recovery_scope_ref, workspace_fs_id, created_at
       FROM runs
       WHERE workspace_id = ?
       ORDER BY created_at DESC`)
        .all(workspace.id);
    for (const run of runRows) {
        if (!latestRuns.has(run.item_id))
            latestRuns.set(run.item_id, run);
    }
    const openPromptsByRun = new Map();
    const promptRows = db
        .prepare(`SELECT p.run_id, p.actions_json
    ...
    for (const row of promptRows) {
        if (!openPromptsByRun.has(row.run_id))
            openPromptsByRun.set(row.run_id, row);
    }
    return {
...
export function getRunTree(repos, runId) {
