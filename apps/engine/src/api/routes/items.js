...
import { isItemAction, lookupTransition } from "../../core/itemActions.js";
...
export function handleListItems(repos, url, res) {
    ...
    if (column)
        items = items.filter(item => item.current_column === column);
    if (status)
        items = items.filter(item => item.phase_status === status);
    if (limit !== undefined)
        items = items.slice(0, limit);
    json(res, 200, { items });
}
export function handleGetItem(repos, res, itemId) {
    const item = repos.getItem(itemId);
    if (!item)
        return json(res, 404, { error: "item_not_found", code: "not_found" });
    json(res, 200, item);
}
function respondInvalidItemAction(res) {
    json(res, 400, {
        error: "bad_request",
        code: "bad_request",
        valid: [
...
function isRunStartingAction(action) {
...
function respondItemActionFailure(res, result) {
    if (result.ok)
        return;
...
function handleStartRunItemAction(repos, res, itemId, action, onItemColumnChanged) {
    const item = repos.getItem(itemId);
    if (!item) {
...
async function maybeResumeItemActionRun(repos, res, result) {
...
export function handleGetItemPreview(repos, res, itemId) {
    void (async () => {
        const context = resolveItemPreviewContext(repos, itemId);
        if (!context.ok)
            return json(res, 404, context);
    ...
    })().catch(err => json(res, 500, { error: err.message, code: "preview_lookup_failed" }));
}
export async function handleStartItemPreview(repos, req, res, itemId) {
...
export async function handleStopItemPreview(repos, req, res, itemId) {
    await readJson(req).catch(() => ({}));
    const context = resolveItemPreviewContext(repos, itemId);
    if (!context.ok)
        return json(res, 404, context);
...
export function handleGetItemWireframes(repos, res, itemId) {
    const item = repos.getItem(itemId);
    if (!item)
        return json(res, 404, { error: "item_not_found", code: "not_found" });
    const run = latestRunForItemWithStageArtifact(repos, item.id, "visual-companion", "wireframes.json");
    if (!run)
        return json(res, 404, { error: "no_design_prep", code: "not_found" });
    const ctx = resolveWorkflowContextForRun(repos, run);
    if (!ctx)
        return json(res, 404, { error: "artefact_root_unreachable", code: "not_found" });
...
export function handleGetItemDesign(repos, res, itemId) {
    const item = repos.getItem(itemId);
    if (!item)
        return json(res, 404, { error: "item_not_found", code: "not_found" });
 ...
 *
 */
export async function handleItemActionNamed(itemActions, repos, req, res, itemId, action, onItemColumnChanged) {
    ...
    if (await maybeResumeItemActionRun(repos, res, result))
        return;
    return json(res, 200, {
        kind: result.kind,
