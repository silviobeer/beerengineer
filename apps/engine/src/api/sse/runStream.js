 ...
 */
export function handleRunEvents(repos, req, res, runId) {
    ...
    const tail = tailStageLogs(repos, { scope: { kind: "run", runId }, sinceId }, row => {
        if (closed || seenStreamIds.has(row.id))
            return;
        const entry = projectStageLogRow(row);
        if (!entry || !shouldDeliverAtLevel(entry, subscribedLevel)) {
            if (row.event_type === "run_finished" || row.event_type === "run_failed" || row.event_type === "run_blocked")
                close();
            return;
        }
        markSeen(row.id);
        writeSse(res, entry.type, entry);
        if (entry.type === "run_finished" || entry.type === "run_failed" || entry.type === "run_blocked")
            close();
    });
