...
function totalMergedStories(state: ProjectReviewState): number {
  return state.executionSummaries.reduce((sum, wave) => sum + wave.storiesMerged.length, 0)
}

function primaryFinding(project: Project, state: ProjectReviewState): ProjectReviewFinding {
...
function lowFinding(): ProjectReviewFinding {
  return {
    id: "PR-MAINT-01",
    source: "project-review-llm",
    severity: "low",
...
function buildArtifact(project: Project, state: ProjectReviewState): ProjectReviewArtifact {
...
export class FakeProjectReviewStageAdapter
  implements StageAgentAdapter<ProjectReviewState, ProjectReviewArtifact> {
  constructor(private readonly project: Project) {}

  async step(input: StageAgentInput<ProjectReviewState>): Promise<StageAgentResponse<ProjectReviewArtifact>> {
