export function mapStageEnvelopeToResponse(envelope) {
...
export function mapReviewEnvelopeToResponse(envelope) {
    switch (envelope.kind) {
        case "pass":
            return { kind: "pass" };
        case "revise":
            if (!envelope.feedback?.trim())
                throw new Error("Hosted review response is missing feedback");
            return { kind: "revise", feedback: envelope.feedback };
        case "block":
            if (!envelope.reason?.trim())
                throw new Error("Hosted review response is missing a block reason");
