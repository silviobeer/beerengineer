...
function isToolUseContent(part) {
...
function buildClaudeCommand(input) {
    ...
    if (mode)
        command.push("--permission-mode", mode);
...
function summarizeClaudeSystemEvent(event, state) {
...
function summarizeClaudeAssistantEvent(event, state) {
    ...
    const toolUse = message.content.find((part) => {
    ...
    if (toolUse?.name) {
        ...
        if (toolUse.id)
            state.toolCalls.set(toolUse.id, { name: toolUse.name, argsPreview });
...
function emitClaudeToolResult(content, state) {
    const toolResult = content?.find((part) => part.type === "tool_result");
    if (!toolResult)
        return;
    const call = toolResult.tool_use_id ? state.toolCalls.get(toolResult.tool_use_id) : undefined;
    emitHostedToolResult(call?.name ?? toolResult.tool_use_id ?? "tool", call?.argsPreview, sanitizePreviewValue(toolResult.content), "claude", toolResult.is_error === true);
}
function summarizeClaudeUserEvent(event, state) {
    emitClaudeToolResult(event.message?.content, state);
}
function summarizeClaudeContentBlockStart(inner, state) {
    if (inner.type !== "content_block_start" || typeof inner.index !== "number")
        return null;
    const blockType = inner.content_block?.type;
    if (typeof blockType === "string")
        state.textBlockTypes.set(inner.index, blockType);
...
function appendClaudeContentDelta(inner, state) {
...
function summarizeClaudeStreamEvent(event, state) {
...
function summarizeClaudeEvent(event, state) {
    if (typeof event.session_id === "string")
        state.sessionId = event.session_id;
    if (event.type === "system")
        return summarizeClaudeSystemEvent(event, state);
    if (event.type === "assistant")
        return summarizeClaudeAssistantEvent(event, state);
    if (event.type === "user") {
        summarizeClaudeUserEvent(event, state);
        return null;
    }
    if (event.type === "stream_event")
        return summarizeClaudeStreamEvent(event, state);
    if (event.type === "result")
        return summarizeClaudeResultEvent(event, state);
    if (event.type === "error") {
        state.streamedSummary = true;
...
function finalOutputText(state) {
...
const claudeDriver = {
    ...
    streamCallback: state => makeJsonLineStreamCallback({
...
export async function invokeClaude(input) {
