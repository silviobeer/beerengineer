...
const webhookWritesByChat = new Map();
function plainStatus(res, status, body) {
...
async function softReply(botToken, chatId, text, sendImpl) {
...
function allowWebhookWrite(chatId) {
...
function inboundFailureMessage(error) {
    switch (error) {
...
export function resetTelegramChatToolWebhookRateLimit() {
    webhookWritesByChat.clear();
}
export async function handleTelegramChatToolWebhook(repos, config, req, res, deps = {}) {
    if (req.method !== "POST")
        return plainStatus(res, 405, "method_not_allowed");
    const resolved = resolveTelegramChatToolConfig(config);
    if (!resolved.enabled)
        return plainStatus(res, 404, resolved.reason);
    if (config.notifications?.telegram?.inbound?.enabled !== true)
        return plainStatus(res, 404, "inbound disabled");
    ...
    if (!update)
        return plainOk(res);
    if (update.channelRef !== resolved.chatId)
        return plainOk(res);
    if (!allowWebhookWrite(update.channelRef))
        return plainOk(res);
    if (!update.text || update.text.startsWith("/")) {
        await softReply(resolved.botToken, update.channelRef, "Reply to a beerengineer_ prompt to answer it. Commands are not supported yet.", deps.send);
        return plainOk(res);
    }
    ...
    if (result.kind === "ignored") {
        await softReply(resolved.botToken, update.channelRef, "Reply to a beerengineer_ prompt to answer it.", deps.send);
        return plainOk(res);
    }
