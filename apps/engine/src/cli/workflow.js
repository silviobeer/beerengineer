...
export async function runInteractiveWorkflow(opts = {}) {
    if (opts.json)
    ...
    try {
        ...
        const runId = await runWorkflowWithSync({ id: "new", title, description }, repos, io, { owner: "cli", ...workspaceMeta });
        console.log(`\n  run-id: ${runId}`);
    }
    finally {
        io.close?.();
...
async function runJsonWorkflow(opts = {}) {
