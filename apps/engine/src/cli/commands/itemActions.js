...
let lookupTransitionSync;
let createItemActionsService;
async function collectRemediationFlags(flags, interactive) {
    const out = { ...flags };
    if (interactive && !out.summary) {
        try {
            out.summary = (await ask("  Remediation summary (required): ")).trim() || undefined;
            if (!out.branch)
                out.branch = (await ask("  Branch (optional):            ")).trim() || undefined;
            if (!out.notes)
                out.notes = (await ask("  Review notes (optional):      ")).trim() || undefined;
        }
        finally {
            close();
        }
...
function printResumeBlockedOutput(runId, recovery, itemRef) {
    const scopeDetail = recovery.scopeRef ? ` (${recovery.scopeRef})` : "";
    console.error(`\n  Run ${runId} is blocked.`);
    if (recovery.summary)
        console.error(`  Reason: ${recovery.summary}`);
    if (recovery.scope)
        console.error(`  Scope:  ${recovery.scope}${scopeDetail}`);
    console.error(`  Resume with: beerengineer item action --item ${itemRef} --action resume_run --remediation-summary "<what you fixed>"`);
}
async function collectResumePayload(active, resumeFlags, itemRef) {
...
function printDirtyRepoPreflight(rootPath) {
    const inspection = inspectWorkspaceState(rootPath);
    if (inspection.kind !== "dirty")
        return 0;
...
function preflightCliBranchingStart(repos, workspaceId) {
...
function seedStageFromPreviousRun(repos, item, sourceRunId, targetRunId, stageId) {
...
const handleStartBrainstorm = async (ctx) => {
    const exit = startRunPrelude(ctx);
    if (exit !== 0)
        return exit;
    const io = createCliIO(ctx.repos);
    try {
        const runId = await runWorkflowWithSync({ id: ctx.item.id, title: ctx.item.title, description: ctx.item.description }, ctx.repos, io, { owner: "cli", itemId: ctx.item.id });
...
const handleStartImplementationOrRerunDesignPrep = async (ctx) => {
    const exit = startRunPrelude(ctx);
    if (exit !== 0)
        return exit;
    ...
    const io = createCliIO(ctx.repos);
    try {
        const prepared = prepareRun({ id: ctx.item.id, title: ctx.item.title, description: ctx.item.description }, ctx.repos, io, {
            owner: "cli",
            itemId: ctx.item.id,
            resume: {
...
const handleStartVisualCompanion = async (ctx) => {
    const exit = startRunPrelude(ctx);
    if (exit !== 0)
        return exit;
    const sourceRun = latestRunWithStageArtifacts(ctx.repos, ctx.item, "brainstorm");
...
const handleStartFrontendDesign = async (ctx) => {
    const exit = startRunPrelude(ctx);
    if (exit !== 0)
        return exit;
...
const handleResumeRun = async (ctx) => {
    const active = ctx.repos.latestActiveRunForItem(ctx.item.id) ?? ctx.repos.latestRecoverableRunForItem(ctx.item.id);
    const resumeRunId = active?.id;
    const resumePayloadResult = await collectResumePayload(active, ctx.resumeFlags, ctx.itemRef);
    if (resumePayloadResult.kind === "exit")
        return resumePayloadResult.code;
...
async function runDefaultItemAction(ctx, resumePayload) {
    const service = createItemActionsService(ctx.repos);
    try {
        const result = await service.perform(ctx.item.id, ctx.action, resumePayload ? { resume: resumePayload } : undefined);
        if (!result.ok) {
        ...
        if (result.kind === "needs_spawn" && result.remediationId)
            console.log(`  remediation-id: ${result.remediationId}`);
        if (result.kind === "needs_spawn" && result.runId) {
...
const CLI_ITEM_ACTION_HANDLERS = {
...
export async function runItemAction(itemRef, action, resumeFlags) {
