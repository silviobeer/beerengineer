...
import { artifactPath, deriveItemStatus, deriveRunStatus, gitState, latestRunForItem, openBrowser, printItemCommandError, promptDisplayText, readArtifactJson, resolveCliItem, resolveItemPreview, resolvePreviewLaunch, resolvePreviewState, resolvePublicBaseUrl, resolveSelectedWorkspace, runRunTail, runSortWeight, shortRunId, truncate, withRepos, buildPreviewPayload, emitJsonLine, isUiReachable, listProjectedMessages, printMessageEntry, itemSortWeight, readAnswerBody, EXIT_USAGE, } from "../common.js";
function jsonOut(payload, pretty = true) {
    process.stdout.write(`${JSON.stringify(payload, null, pretty ? 2 : 0)}\n`);
    return 0;
}
function missingItemRef(verb) {
    console.error(`  Missing item reference: beerengineer ${verb} <id|code>`);
    return 2;
}
function notFoundItem(itemRef) {
    console.error(`  Item not found: ${itemRef}`);
    return 1;
}
function findItemByRef(repos, workspaceId, ref) {
    return repos.getItemByCode(workspaceId, ref) ?? repos.getItem(ref);
}
function workspaceState(runs, hasOpenPrompts) {
    if (hasOpenPrompts)
        return "needs_answer";
    if (runs.some(r => r.recovery_status === "blocked"))
        return "blocked";
    if (runs.some(r => r.status === "running"))
        return "running";
    return "idle";
}
async function openOrPrintUrl(url) {
    console.log(`  ${url}`);
    if (await isUiReachable(url))
        openBrowser(url);
    else
        console.log("  UI is not reachable on that address; printed URL only.");
}
function resolveItemCompletedRun(repos, itemRef, workspaceKey, json) {
    const workspace = resolveSelectedWorkspace(repos, workspaceKey);
    if (!workspace)
        return { ok: false, code: 2 };
    const item = findItemByRef(repos, workspace.id, itemRef);
    if (!item)
        return { ok: false, code: printItemCommandError(json, 2, `item not found: ${itemRef}`, `  Item not found: ${itemRef}`) };
    const run = latestCompletedRunForItem(repos, item.id);
    if (!run)
        return { ok: false, code: printItemCommandError(json, 3, `no completed run for ${item.code}`, `  no completed run for ${item.code}`) };
    return { ok: true, item, run };
}
function maybeOpenFile(path, open) {
    if (open)
        openBrowser(`file://${path}`);
}
function printItemRows(rows, compact = false) {
    if (compact) {
...
export async function runItemsCommand(workspaceKey, all = false, json = false, compact = false) {
...
export async function runWorkspaceItemsCommand(key, json = false) {
    if (!key) {
...
function printChatRows(rows, compact = false) {
    if (compact) {
...
export async function runChatListCommand(workspaceKey, all = false, json = false, compact = false) {
    return withRepos(async (repos) => {
...
export async function runStatusCommand(workspaceKey, all = false, json = false) {
    return withRepos(async (repos) => {
        if (all) {
        ...
        if (json)
            return jsonOut({ workspace: workspace.key, state, itemCount: items.length, runCount: runs.length, chatCount: openPrompts.length });
        ...
        if (latestRun)
            console.log(`  latest run: run/${shortRunId(latestRun.id)} / ${latestRun.current_stage ?? "—"} / ${deriveRunStatus(latestRun, openPrompts.some(prompt => prompt.run_id === latestRun.id))}`);
        return 0;
    });
}
export async function runItemGetCommand(itemRef, workspaceKey, json = false) {
    if (!itemRef)
        return missingItemRef("item get");
    return withRepos(async (repos) => {
        const workspace = resolveSelectedWorkspace(repos, workspaceKey);
        if (!workspace)
            return 1;
        ...
        if (json)
            return jsonOut({ item, workspace: workspace.key, latestRun, status, openChat });
        console.log(`  ${item.code}  ${item.title}`);
        console.log(`  workspace: ${workspace.key}`);
        console.log(`  stage/status: ${latestRun?.current_stage ?? item.current_column} / ${status}`);
        if (latestRun)
            console.log(`  run: ${latestRun.id} (${latestRun.status})`);
        if (openChat)
            console.log(`  open chat: ${openChat}`);
        return 0;
    });
}
export async function runItemWireframesCommand(itemRef, workspaceKey, open = false, json = false) {
    if (!itemRef)
        return missingItemRef("item wireframes");
    return withRepos(async (repos) => {
        const workspace = resolveSelectedWorkspace(repos, workspaceKey);
        if (!workspace)
            return 2;
        ...
        const artifact = readArtifactJson(repos, run.id, "visual-companion", "wireframes.json");
        if (!artifact) {
        ...
        if (json) {
            ...
            if (open)
                openBrowser(`file://${screenMapPath}`);
            return 0;
        }
...
export async function runItemDesignCommand(itemRef, workspaceKey, open = false, json = false) {
    if (!itemRef)
        return missingItemRef("item design");
    return withRepos(async (repos) => {
        const workspace = resolveSelectedWorkspace(repos, workspaceKey);
        if (!workspace)
            return 2;
        ...
        if (!run)
            return printItemCommandError(json, 3, `no completed run for ${item.code}`, `  no completed run for ${item.code}`);
        const artifact = readArtifactJson(repos, run.id, "frontend-design", "design.json");
        if (!artifact) {
            return printItemCommandError(json, 3, `no design-prep artifacts for ${item.code} (hasUi=false)`, `  no design-prep artifacts for ${item.code} (hasUi=false)`);
        }
...
export async function runRunListCommand(workspaceKey, all = false, json = false, compact = false) {
    return withRepos(async (repos) => {
        ...
        const items = new Map((selected ? repos.listItemsForWorkspace(selected.id) : Array.from(workspaces.keys()).flatMap(id => repos.listItemsForWorkspace(id)))
...
export async function runRunGetCommand(runId, json = false) {
    if (!runId) {
...
export async function runChatAnswerCommand(cmd) {
    const { recordAnswer } = await import("../../core/conversation.js");
    return withRepos(async (repos) => {
        let prompt;
        if (cmd.promptId)
            prompt = repos.getPendingPrompt(cmd.promptId);
        else if (cmd.runId)
            prompt = repos.getOpenPrompt(cmd.runId);
        ...
        if (cmd.json)
            return jsonOut(result.conversation);
        console.log(`  answered ${prompt.id}`);
        console.log(`  run: ${prompt.run_id}`);
        if (cmd.runId)
            console.log("  target: latest open prompt for run");
        return 0;
    });
}
export async function runChatSendCommand(cmd) {
    ...
    return withRepos(async (repos) => {
        ...
        if (cmd.json)
            return jsonOut(entry ?? { id: result.entryId, runId: cmd.runId });
        console.log(`  sent ${result.entryId}`);
        console.log(`  run: ${cmd.runId}`);
        return 0;
    });
}
export async function runRunMessagesCommand(cmd) {
    if (!cmd.runId) {
    ...
    return withRepos(async (repos) => {
        ...
        if (cmd.json)
            return jsonOut({ runId: cmd.runId, schema: "messages-v1", nextSince: result.nextSince, entries: result.entries });
        console.log(`  messages ${run.id}  ${run.title}`);
        result.entries.forEach(printMessageEntry);
        return 0;
    });
}
export async function runRunTailCommand(cmd) {
    if (!cmd.runId) {
...
export async function runRunWatchCommand(cmd) {
...
export async function runItemOpenCommand(itemRef, workspaceKey) {
    if (!itemRef)
        return missingItemRef("item open");
    return withRepos(async (repos) => {
        const workspace = resolveSelectedWorkspace(repos, workspaceKey);
        if (!workspace)
            return 1;
...
export async function runItemPreviewCommand(itemRef, workspaceKey, opts) {
    if (!itemRef)
        return missingItemRef("item preview");
    ...
    return withRepos(async (repos) => {
        ...
        if (opts.open && (payload.status === "started" || payload.status === "already_running"))
            openBrowser(preview.previewUrl);
        return 0;
    });
}
export async function runRunOpenCommand(runId) {
    if (!runId) {
