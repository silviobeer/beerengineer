...
import { backfillWorkspaceConfigs, getRegisteredWorkspace, listRegisteredWorkspaces, openWorkspace, previewWorkspace, promptForWorkspaceAddDefaults, registerWorkspace, removeWorkspace, } from "../../core/workspaces.js";
import { generateSetupReport } from "../../setup/doctor.js";
import { confirmWorkspacePurge, indentBlock, loadEffectiveConfig, parseHarnessProfile, printPreview, saveCliState, withRepos, } from "../common.js";
export async function runWorkspacePreviewCommand(path, json = false) {
...
export async function runWorkspaceAddCommand(cmd) {
    const config = loadEffectiveConfig();
    if (!config) {
        console.error("  App config is missing or invalid. Run `beerengineer setup` first.");
        return 1;
...
async function resolveWorkspaceAddInput(cmd, config) {
...
async function executeWorkspaceAdd(cmd, config, addInput) {
    return withRepos(async (repos) => {
...
function printWorkspaceAddResult(result) {
    ...
    if (result.workspace.sonarEnabled)
        printWorkspaceSonarReadiness(result);
    if (result.sonarProjectUrl || result.ghCreateCommand)
        printWorkspaceCodeRabbitHints(result.ghCreateCommand);
...
function printWorkspaceCodeRabbitHints(ghCreateCommand) {
    ...
    if (ghCreateCommand)
        console.log(`    Optional remote: ${ghCreateCommand}`);
}
export async function runWorkspaceListCommand(json = false) {
    return withRepos(async (repos) => {
...
export async function runWorkspaceGetCommand(key, json = false) {
    if (!key) {
    ...
    return withRepos(async (repos) => {
        ...
        if (json)
            ...
            console.log(`  ${workspace.key}  ${workspace.rootPath}`);
        return 0;
    });
}
export async function runWorkspaceRemoveCommand(key, purge = false, json = false, yes = false, noInteractive = false) {
    if (!key) {
...
export async function runWorkspaceOpenCommand(key) {
    if (!key) {
...
export async function runWorkspaceUseCommand(key) {
    if (!key) {
...
export async function runWorkspaceBackfillCommand(json = false) {
    return withRepos(async (repos) => {
        ...
        for (const key of result.written)
            console.log(`  wrote config for ${key}`);
        for (const skipped of result.skipped)
            console.log(`  skipped ${skipped.key}: ${skipped.reason}`);
        return 0;
    });
}
export async function runWorkspaceWorktreeGcCommand(key, json = false) {
    if (!key) {
