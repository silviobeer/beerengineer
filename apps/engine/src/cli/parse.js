...
function isKnownGroupId(group) {
    return KNOWN_GROUP_IDS.includes(group);
}
function parseRunSubcommand(context) {
    ...
    if (second === "list")
        return { kind: "run-list", workspaceKey, json, all, compact };
    if (second === "get")
        return { kind: "run-get", runId: argv[2], json };
    if (second === "open")
        return { kind: "run-open", runId: argv[2] };
    if (second === "tail")
        return { kind: "run-tail", runId: argv[2], level, since, json };
    if (second === "messages")
        return { kind: "run-messages", runId: argv[2], level: messagesLevel, since, limit, json };
    if (second === "watch")
        return { kind: "run-watch", runId: argv[2], level, since, json };
    return { kind: "unknown", token: argv.join(" ") };
}
function parseWorkspaceSubcommand(context) {
    const { first, second, argv, json } = context;
    if (first !== "workspace")
        return null;
    if (second === "preview")
        return { kind: "workspace-preview", path: argv[2], json };
    if (second === "add") {
        return {
            kind: "workspace-add",
            json,
    ...
    if (second === "list")
        return { kind: "workspace-list", json };
    if (second === "get")
        return { kind: "workspace-get", key: argv[2], json };
    if (second === "items")
        return { kind: "workspace-items", key: argv[2], json };
    if (second === "use")
        return { kind: "workspace-use", key: argv[2] };
    if (second === "remove") {
        return {
    ...
    if (second === "open")
        return { kind: "workspace-open", key: argv[2] };
    if (second === "backfill")
        return { kind: "workspace-backfill", json };
    if (second === "gc-worktrees")
        return { kind: "workspace-worktree-gc", key: argv[2], json };
    return null;
}
function parseChatSubcommand(context) {
    const { first, second, argv, json, workspaceKey, all, compact, positionalThird } = context;
    if (first === "chat" && second === "list")
        return { kind: "chat-list", workspaceKey, json, all, compact };
...
function resolveResumeFlags(argv) {
    const resume = {};
...
function parseItemActionSubcommand(argv) {
...
function parseItemSubcommand(context) {
    const { first, second, argv, workspaceKey, json } = context;
    if (first !== "item")
        return null;
    if (second === "get")
        return { kind: "item-get", itemRef: argv[2], workspaceKey, json };
    if (second === "open")
        return { kind: "item-open", itemRef: argv[2], workspaceKey };
    if (second === "preview")
        return { kind: "item-preview", itemRef: argv[2], workspaceKey, start: argv.includes("--start"), stop: argv.includes("--stop"), open: argv.includes("--open"), json };
    if (second === "wireframes")
        return { kind: "item-wireframes", itemRef: argv[2], workspaceKey, open: argv.includes("--open"), json };
    if (second === "design")
        return { kind: "item-design", itemRef: argv[2], workspaceKey, open: argv.includes("--open"), json };
    if (second === "action")
        return parseItemActionSubcommand(argv);
    return null;
}
function buildParseArgsContext(argv) {
...
function parseUpdateCommand(context) {
    if (context.first !== "update")
        return null;
    return {
        kind: "update",
...
function parsePluralListCommand(context) {
    const { first, second, argv, workspaceKey, json, all, compact, level, messagesLevel, since, limit } = context;
    if (first === "items")
        return { kind: "items", workspaceKey, json, all, compact };
    if (first === "chats")
        return { kind: "chats", workspaceKey, json, all, compact };
    if (first !== "runs")
        return null;
    if (second === "tail")
        return { kind: "run-tail", runId: argv[2], level, since, json };
...
function parseSimpleTopLevelCommand(context) {
    const { first, second, argv, json, group, workspaceKey, all } = context;
    if (first === "--help" || first === "-h" || first === "help")
        return { kind: "help" };
    if (first === "--doctor" || first === "doctor")
        return { kind: "doctor", json, group };
    if (first === "setup")
        return { kind: "setup", group, noInteractive: argv.includes("--no-interactive") };
    const update = parseUpdateCommand(context);
    if (update)
        return update;
    if (first === "notifications" && second === "test" && argv[2] === "telegram")
        return { kind: "notifications-test", channel: "telegram" };
    if (first === "status")
        return { kind: "status", workspaceKey, json, all };
    const pluralList = parsePluralListCommand(context);
    if (pluralList)
        return pluralList;
    if (first === "start" && second === undefined)
        return { kind: "start-engine" };
    if (first === "start" && second === "ui")
        return { kind: "start-ui" };
    return null;
}
