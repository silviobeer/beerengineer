...
function parseProvider(value) {
    if (value === "anthropic" || value === "openai" || value === "opencode")
        return value;
    return undefined;
}
...
function validateOpenCodeHarnessProfile(roles) {
...
function validateSelfHarnessProfile(roles) {
...
function requireProviderRoles(roles, context) {
    ...
    if (typeof roles.coder.provider !== "string"
...
function requireHarnessRole(role, context) {
    const isHarness = (value) => value === "claude" || value === "codex" || value === "opencode";
    const isRuntime = (value) => value === undefined || value === "cli" || value === "sdk";
...
function validatePublicBaseUrl(publicBaseUrl) {
...
function validateLlmConfig(llm) {
    if (!llm || typeof llm !== "object")
        throw new TypeError("llm config is required");
    const parsed = llm;
...
function validateNotificationsConfig(notifications) {
...
function validateTelegramConfig(telegram) {
...
function validateOptionalRequiredString(value, field) {
    if (value === undefined)
        return undefined;
...
function validateTelegramLevel(value) {
    if (value === undefined)
        return undefined;
...
export function readConfigFile(configPath = defaultConfigPath()) {
    ...
    try {
        ...
        return { kind: "invalid", path: configPath, error: err.message };
    }
}
function envOverrides() {
    ...
    return {
        ...
        telegramLevel: process.env.BEERENGINEER_TELEGRAM_LEVEL === "0" || process.env.BEERENGINEER_TELEGRAM_LEVEL === "1" || process.env.BEERENGINEER_TELEGRAM_LEVEL === "2"
            ? Number(process.env.BEERENGINEER_TELEGRAM_LEVEL)
            : undefined,
    ...
    return {
        ...
        notifications: {
            telegram: {
                ...
                inbound: {
                    enabled: overrides.telegramInboundEnabled ?? base.notifications?.telegram?.inbound?.enabled ?? false,
                    webhookSecretEnv: overrides.telegramWebhookSecretEnv ?? base.notifications?.telegram?.inbound?.webhookSecretEnv,
                },
            },
        },
        vcs: {
...
export function resolveConfiguredDbPath(config) {
