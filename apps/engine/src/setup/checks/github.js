...
export async function runGitHubChecks(enabled) {
    const gh = await probeCommand("gh", ["--version"]);
    const ghCheck = createCheck("vcs.gh", "GitHub CLI", gh.ok ? "ok" : "missing", gh.version ?? gh.detail, { remedy: gh.ok ? undefined : remedyForTool("gh") });
    if (!enabled) {
        return [
            ghCheck,
