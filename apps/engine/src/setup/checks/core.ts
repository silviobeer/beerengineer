...
import type { AppConfig, CheckResult, ConfigFileState } from "../types.js"
...
export async function runCoreChecks(
  ...
  if (preconditionChecks) return [...checks, ...preconditionChecks]
...
function checkConfiguredDataDir(dataDir: string): CheckResult {
  if (!existsSync(dataDir)) return createCheck("core.dataDir", "configured data dir", "missing", dataDir)
  try {
