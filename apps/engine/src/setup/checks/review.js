...
export async function runReviewChecks() {
    ...
    return [
        ...
        createCheck("review.sonar-token", "SONAR_TOKEN", sonarTokenPresent ? "ok" : "missing", sonarTokenPresent
            ? "Token available for scanner/API auth"
            : "Missing SONAR_TOKEN. Story-branch Sonar gating will fail if Sonar is enabled.", sonarTokenPresent
            ? {}
            : { remedy: { hint: "Create a SonarQube Cloud analysis token and export it as SONAR_TOKEN." } }),
        createCheck("review.sonar-plan", "Sonar branch-analysis plan tier", "unknown", "Branch analysis for story branches requires SonarQube Cloud Team, Enterprise, or OSS; Free plan workspaces will be skipped."),
    ];
}
