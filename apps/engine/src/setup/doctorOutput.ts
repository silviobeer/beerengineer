import type { CheckResult, GroupResult, SetupReport, SetupStatus } from "./types.js"

export function summarizeOverall(groups: GroupResult[]): SetupReport["overall"] {
  if (groups.some(group => group.level === "required" && !group.satisfied)) return "blocked"
  return groups.some(group => group.level !== "required" && !group.ideal) ? "warning" : "ok"
}

export function doctorExitCode(report: SetupReport): number {
  return report.overall === "blocked" ? 1 : 0
}

export function printDoctorReport(report: SetupReport, opts: { installHints: boolean }): void {
  console.log("")
  console.log(`  Setup status: ${report.overall}`)
  for (const group of report.groups) {
    console.log(`  ${formatGroupTitle(group)} [${group.passed}/${group.idealOk ?? group.minOk}]`)
    for (const check of group.checks) printDoctorCheck(check, opts.installHints)
  }
  console.log("")
}
