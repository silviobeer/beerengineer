...
import { createGitAdapter } from "./core/gitAdapter.js";
...
import { PROJECT_STAGE_REGISTRY, shouldRunProjectStage, } from "./core/projectStageRegistry.js";
 ...
 */
function loadItemWorkspaceReferences(context) {
...
async function blockRunForWorkspaceState(context, summary, opts = {}) {
...
function currentGitBranch(workspaceRoot) {
...
function normalizeExecutionResume(stageId) {
...
function normalizeProjectResume(input) {
    const scope = input.scope;
    let stageId = "execution";
    if (scope.type === "stage")
        stageId = scope.stageId;
    else if (scope.type === "run")
        stageId = input.currentStage ?? "";
...
function normalizeItemResume(input) {
    const scope = input.scope;
...
async function loadProjects(context) {
    return readJson(join(layout.stageArtifactsDir(context, "brainstorm"), "projects.json"));
}
async function loadConcept(context) {
    try {
        return await readJson(join(layout.stageArtifactsDir(context, "brainstorm"), "concept.json"));
    }
    catch {
...
const loadWireframes = (context) => readJson(join(layout.stageArtifactsDir(context, "visual-companion"), "wireframes.json"));
const loadDesign = (context) => readJson(join(layout.stageArtifactsDir(context, "frontend-design"), "design.json"));
async function loadDesignPrepFreeze(context) {
    try {
        return await readJson(join(layout.stageArtifactsDir(context, "visual-companion"), "project-freeze.json"));
    }
    catch {
        return null;
    }
...
function sameProjectSet(left, right) {
    if (left.length !== right.length)
        return false;
    return left.every((value, index) => value === right[index]);
}
async function assertDesignPrepProjectFreeze(context, projects) {
    ...
    if (sameProjectSet(currentIds, frozenIds))
        return;
    const summary = `Resume blocked: brainstorm project set changed after design prep. ` +
...
export async function runWorkflow(item, options) {
    ...
    try {
        ...
        const { wireframes, design } = await resolveDesignPrepArtifacts(context, itemResumePlan, itemHasUi, itemConcept, projects, options?.llm?.stage, itemSnapshot);
...
async function resolveWorkflowProjects(item, context, git, itemResumePlan, stageLlm, codebaseSnapshot) {
...
function emitWorkflowPreviewPort(context, activeRun) {
...
async function resolveDesignPrepArtifacts(context, itemResumePlan, itemHasUi, itemConcept, projects, stageLlm, itemSnapshot) {
    if (!itemHasUi)
        return { wireframes: undefined, design: undefined };
    ...
    const wireframes = shouldRunVisualCompanion
        ? await withStageLifecycle("visual-companion", () => visualCompanion(context, { itemConcept, projects, references }, stageLlm, itemSnapshot), {})
        : await loadWireframes(context);
    const design = shouldRunFrontendDesign
        ? await withStageLifecycle("frontend-design", () => frontendDesign(context, { itemConcept, projects, wireframes, references }, stageLlm, itemSnapshot), {})
        : await loadDesign(context);
    return { wireframes, design };
}
function emitWorkflowProjectsCreated(activeRun, projects) {
    if (!activeRun)
        return;
...
async function runWorkflowProjects(options) {
    ...
    for (const project of projects) {
        ...
        await runProject({
            ...
            decisions,
        }, git, resumePlan ?? undefined, llm);
    }
}
async function handleWorkflowFailure(context, itemTitle, error) {
    const message = error instanceof Error ? error.message : String(error);
    if (isWorktreePortPoolExhaustedError(error)) {
        const summary = `Run blocked: no preview port is available for "${itemTitle}". ` +
    ...
    if (message.startsWith("git:") || message.startsWith("branch_gate:")) {
        const summary = `Run blocked: git branch gate failed for "${itemTitle}". ` +
            `${message.replace(/^branch_gate:\s*/, "").replace(/^git:\s*/, "")}`;
        stagePresent.warn(summary);
        await blockRunForWorkspaceState(context, summary);
    }
}
function restoreWorkflowExitState(git) {
    try {
        const exitBranch = git.exitRunToItemBranch();
        stagePresent.dim(`→ Run exit branch: ${exitBranch}`);
    }
    catch (error) {
        stagePresent.warn(`Run exit branch restore failed: ${error.message}`);
    }
    try {
        git.assertWorkspaceRootOnBaseBranch("run exit");
    }
    catch (error) {
        stagePresent.warn(`Workspace root invariant failed at run exit: ${error.message}`);
    }
}
async function ensureWorkflowGitAdapter(context, workspaceRoot) {
    try {
        return createGitAdapter(context);
    }
    catch (error) {
        const reason = error.message.replace(/^git:\s*/, "");
...
function workflowGitFailureSummary(reason, workspaceRoot) {
    if (!workspaceRoot)
...
const buildItemSnapshotWithUi = (codebaseSnapshot, workspaceRoot) => (codebaseSnapshot
    ? { ...codebaseSnapshot, frontend: loadFrontendSnapshot(workspaceRoot) }
    : codebaseSnapshot);
const buildItemSnapshotWithoutUi = (codebaseSnapshot) => codebaseSnapshot;
function buildDesignPrepPlan(context, itemResumePlan) {
    ...
    const shouldRunVisualCompanion = isManualVisual || (!itemResumePlan.manualStage && (itemResumePlan.startStage === "brainstorm" ||
        itemResumePlan.startStage === "visual-companion" ||
        !wireframesFileExists));
    const shouldRunFrontendDesign = isManualFrontend || (!itemResumePlan.manualStage && (shouldRunVisualCompanion ||
