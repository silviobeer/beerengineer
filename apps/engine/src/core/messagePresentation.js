function payloadString(value, fallback = "—") {
    return typeof value === "string" && value.trim() ? value : fallback;
}
function payloadNumberString(value, fallback) {
    return typeof value === "number" ? String(value) : fallback;
}
function branchTransitionDetail(entry) {
    return `${payloadString(entry.payload.itemBranch)} → ${payloadString(entry.payload.baseBranch)}`;
}
function iterationLabel(n, phase) {
    switch (phase) {
        case "review":
            return {
                icon: "↻",
...
export function presentMessageEntry(entry) {
    ...
    switch (entry.type) {
        ...
        case "wave_serialized": {
            const stories = Array.isArray(entry.payload.stories) ? entry.payload.stories.join(", ") : "";
            return {
                icon: "↺",
