...
function fileHasConflictMarkers(root, file) {
    try {
...
function buildPrompt(message, files, expectedSharedFiles = []) {
    return [
...
function buildCommandForProvider(harness, model, workspaceRoot, prompt) {
    switch (harness) {
        case "claude": {
            const command = [
                "claude",
        ...
        case "codex": {
            ...
            if (model)
                command.push("-c", `model=${JSON.stringify(model)}`);
            command.push(prompt);
            return { ok: true, command };
        }
        case "opencode":
 ...
 */
function writeResolverLog(logDir, payload) {
    if (!logDir)
        return;
    try {
...
export function resolveMergeConflictsViaLlm(input) {
    ...
    if (input.harness.runtime === "sdk") {
        return {
            ok: false,
            reason: `merge-resolver: ${input.harness.harness}:sdk is not implemented (the resolver runs synchronously inside the git adapter; SDK adapters are async). ` +
    ...
    const built = buildCommandForProvider(input.harness.harness, input.harness.model, input.workspaceRoot, prompt);
    if (!built.ok)
        return built;
