...
export { attachDbSync } from "./dbSync.js";
 ...
 */
export function prepareRun(item, repos, io, opts = {}) {
    ...
    const start = async () => {
        ...
        try {
            await runWithWorkflowIO(io, async () => runWithActiveRun({ runId: runRow.id, itemId: itemRow.id, title: item.title }, async () => {
                ...
                try {
                    ...
                    if (finalRun?.recovery_status === "blocked")
                        return;
                    await persistWorkflowRunState({ workspaceId, runId: runRow.id, workspaceRoot: workspaceRow?.root_path ?? undefined }, finalRun?.current_stage ?? "handoff", "completed");
                    bus.emit({
                        type: "run_finished",
                        runId: runRow.id,
                        itemId: itemRow.id,
                    ...
                    const finalRun = repos.getRun(runRow.id);
                    if (finalRun?.recovery_status !== "blocked") {
                        await persistWorkflowRunState({ workspaceId, runId: runRow.id, workspaceRoot: workspaceRow?.root_path ?? undefined }, finalRun?.current_stage ?? "execution", "failed");
                        bus.emit({
                            type: "run_finished",
                            runId: runRow.id,
                            itemId: itemRow.id,
...
export async function runWorkflowWithSync(item, repos, io, opts = {}) {
