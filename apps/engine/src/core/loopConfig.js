 ...
 */
export const RALPH_LOOP_DEFAULTS = {
 ...
 */
function normalizePositiveIntOverride(value, name) {
    if (value === undefined)
        return undefined;
    if (!Number.isFinite(value) || !Number.isInteger(value) || value <= 0) {
        if (!warnedFor.has(`override-invalid:${name}`)) {
            warnedFor.add(`override-invalid:${name}`);
            process.stderr.write(`[loopConfig] override ${name}=${value} is not a positive integer; ignoring and falling back to env/default\n`);
        }
        return undefined;
    }
    return value;
...
function clampWithWarning(value, cap, source, name) {
    if (value <= cap)
        return value;
    ...
    if (!warnedFor.has(key)) {
        warnedFor.add(key);
        process.stderr.write(`[loopConfig] ${name}=${value} from ${source} exceeds cap ${cap}; clamping to ${cap}\n`);
    }
    return cap;
}
/**
 ...
 */
export function resolveRalphLoopConfig(override) {
    const fromEnv = {
        maxIterationsPerCycle: clampWithWarning(readPositiveIntEnv("BEERENGINEER_MAX_ITERATIONS_PER_CYCLE", RALPH_LOOP_DEFAULTS.maxIterationsPerCycle), RALPH_LOOP_CAPS.maxIterationsPerCycle, "env", "BEERENGINEER_MAX_ITERATIONS_PER_CYCLE"),
        maxReviewCycles: clampWithWarning(readPositiveIntEnv("BEERENGINEER_MAX_REVIEW_CYCLES", RALPH_LOOP_DEFAULTS.maxReviewCycles), RALPH_LOOP_CAPS.maxReviewCycles, "env", "BEERENGINEER_MAX_REVIEW_CYCLES"),
    };
    ...
    const overrideIter = normalizePositiveIntOverride(override?.maxIterationsPerCycle, "maxIterationsPerCycle");
    const overrideCycles = normalizePositiveIntOverride(override?.maxReviewCycles, "maxReviewCycles");
    return {
        maxIterationsPerCycle: overrideIter === undefined
            ? fromEnv.maxIterationsPerCycle
            : clampWithWarning(overrideIter, RALPH_LOOP_CAPS.maxIterationsPerCycle, "override", "maxIterationsPerCycle"),
        maxReviewCycles: overrideCycles === undefined
            ? fromEnv.maxReviewCycles
            : clampWithWarning(overrideCycles, RALPH_LOOP_CAPS.maxReviewCycles, "override", "maxReviewCycles"),
    };
}
/** Test-only: clear the de-dup state of the cap warning. */
export function _resetLoopConfigWarnings() {
    warnedFor.clear();
}
