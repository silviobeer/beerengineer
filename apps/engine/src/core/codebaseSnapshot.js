...
function shouldSkipTreeEntry(name, depth) {
    if (SKIP_DIRS.has(name))
        return true;
    return name.startsWith(".") && depth === TREE_DEPTH;
}
function readTreeEntryKind(path) {
    try {
...
function walkTree(root, current, depth, out) {
    if (depth < 0)
        return;
    let entries;
    try {
    ...
    for (const name of entries) {
        if (shouldSkipTreeEntry(name, depth))
            continue;
        ...
        if (kind === "dir")
            walkTree(root, full, depth - 1, out);
    }
}
/**
 ...
 */
export function loadCodebaseSnapshot(workspaceRoot) {
    ...
    for (const name of [...TOP_LEVEL_FILE_NAMES, ...WORKSPACE_DOC_FILE_NAMES]) {
        ...
        if (content !== undefined)
            topLevelFiles.push({ path: name, content });
    }
    let openApiSpec;
