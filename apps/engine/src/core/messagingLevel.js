function isFinalFacingAgentMessage(event) {
...
export function levelOf(event) {
    switch (event.type) {
        ...
        case "chat_message":
            if (event.role === "user")
                return { level: 1, force: false, type: "user_message" };
            return {
...
export function messagingLevelFromQuery(input, defaultLevel) {
    if (!input)
        return defaultLevel;
    const normalized = input.trim().toUpperCase();
    if (normalized === "L0" || normalized === "0")
        return 0;
    if (normalized === "L1" || normalized === "1")
        return 1;
    if (normalized === "L2" || normalized === "2")
        return 2;
    return defaultLevel;
}
export function shouldDeliverAtLevel(entry, subscribedLevel) {
