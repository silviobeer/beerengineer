...
import { levelOf } from "./messagingLevel.js";
function parseRecoveryScope(value, runId) {
...
const STAGE_LOG_EVENT_PARSERS = {
    ...
    loop_iteration: (row, data) => ({
        ...
        phase: data.phase === "user-message" || data.phase === "review-feedback" || data.phase === "review"
            ? data.phase
            : "begin",
    ...
    chat_message: (row, data) => ({
        ...
        source: data.source === "reviewer" || data.source === "system" || data.source === "cli" || data.source === "api" || data.source === "webhook"
            ? data.source
            : "stage-agent",
        text: row.message,
    ...
    presentation: (row, data) => ({
        ...
        runId: row.run_id,
        stageRunId: row.stage_run_id ?? null,
        kind: data.kind === "header" || data.kind === "step" || data.kind === "ok" || data.kind === "warn" || data.kind === "dim" || data.kind === "finding"
            ? data.kind
            : "dim",
        text: row.message,
    ...
    wave_serialized: (row, data) => ({
        ...
        stories: Array.isArray(data.stories) ? data.stories.filter((value) => typeof value === "string") : [],
        overlappingFiles: Array.isArray(data.overlappingFiles) ? data.overlappingFiles.filter((value) => typeof value === "string") : [],
        cause: data.cause === "missing_shared_files" ? "missing_shared_files" : "shared_file_overlap",
    }),
};
function eventFromStageLog(row) {
...
function payloadOf(event, rawType) {
    return { ...event, rawType };
}
export function projectWorkflowEvent(event, input) {
...
export function projectStageLogRow(row) {
