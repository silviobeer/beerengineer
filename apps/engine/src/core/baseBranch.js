...
export function isEngineOwnedBranchName(branch) {
    return /^(item|proj|wave|story|candidate)\//.test(branch);
}
function resolveGitDefaultBranch(workspaceRoot) {
    ...
    if (branch && !isEngineOwnedBranchName(branch))
        return branch;
    return null;
}
function readConfiguredBaseBranch(workspaceRoot) {
    try {
        const configPath = resolve(workspaceRoot, ".beerengineer", "workspace.json");
        const raw = readFileSync(configPath, "utf8");
        const parsed = JSON.parse(raw);
        return (parsed.preflight?.github?.defaultBranch?.trim() ||
...
export function resolveBaseBranchForWorkspace(workspaceRoot) {
    const envOverride = process.env.BEERENGINEER_BASE_BRANCH?.trim();
    if (envOverride)
        return { branch: envOverride, source: "env" };
    if (workspaceRoot) {
        ...
        if (fromGit)
            return { branch: fromGit, source: "git" };
    }
    return { branch: "main", source: "default" };
}
export function resolveBaseBranchForItem(itemOverride, workspaceRoot) {
    const trimmed = itemOverride?.trim();
    if (trimmed)
        return { branch: trimmed, source: "item" };
