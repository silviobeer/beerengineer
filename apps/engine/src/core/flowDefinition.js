...
export const ALL_FLOWS = [...ITEM_FLOW, ...PROJECT_FLOW];
/** Adjacency edges for the project flow, derived from `dependsOn`. */
export function projectFlowEdges() {
    return PROJECT_FLOW.flatMap(node => node.dependsOn.map(from => ({ from, to: node.id })));
}
/** Adjacency edges for the item flow, derived from `dependsOn`. */
export function itemFlowEdges() {
    return ITEM_FLOW.flatMap(node => node.dependsOn.map(from => ({ from, to: node.id })));
}
