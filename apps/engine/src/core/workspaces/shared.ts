...
export function sonarPropertiesPath(root: string): string {
...
export function sonarWorkflowPath(root: string): string {
  return resolve(root, SONAR_WORKFLOW_FILE)
}

export function coderabbitConfigPath(root: string): string {
...
export function parseSonarProperties(raw: string): SonarProperties {
...
export async function collectGlobMatches(iterator: AsyncIterable<string>): Promise<string[]> {
  const matches: string[] = []
  for await (const match of iterator) matches.push(match)
  return matches
}

export async function expandWorkspacePattern(root: string, pattern: string): Promise<string[]> {
...
export async function listWorkspacePackageFiles(root: string, rawWorkspaces: unknown): Promise<string[]> {
  let patterns: string[] = []
...
export function packageLooksLikeCoverageProducer(pkg: PackageJsonShape | null): string[] {
  if (!pkg) return []
  const hits: string[] = []
...
export async function isWritablePath(path: string): Promise<boolean> {
  try {
...
export async function findWritableParent(path: string): Promise<boolean> {
...
export function isInsideAllowedRoot(path: string, allowedRoots: string[]): boolean {
...
export function runCommand(command: string, args: string[], cwd: string): { ok: boolean; stdout: string; stderr: string } {
...
export function readEnvFileValue(raw: string, key: string): string | undefined {
  for (const line of raw.split(/\r?\n/)) {
    const trimmed = line.trim()
    if (!trimmed || trimmed.startsWith("#")) continue
...
function isEngineOwnedBranch(branch: string): boolean {
  return ["item/", "proj/", "wave/", "story/", "candidate/"].some(prefix => branch.startsWith(prefix))
}

export function resolveGitDefaultBranch(root: string): string | null {
  const originHead = runGit(["symbolic-ref", "--quiet", "--short", "refs/remotes/origin/HEAD"], root)
  if (originHead.ok && originHead.stdout) return originHead.stdout.replace(/^origin\//, "") || null

  const remoteShow = runGit(["remote", "show", "origin"], root)
  if (remoteShow.ok) {
    const branch = remoteShow.stdout
      .split(/\r?\n/)
      .map(line => line.trim())
  ...
  for (const candidate of ["main", "master"]) {
    if (runGit(["rev-parse", "--verify", "--quiet", `refs/heads/${candidate}`], root).ok) return candidate
  }
  return null
}
...
export function persistSonarTokenToGitConfig(root: string, token: string): void {
...
export function safeParseHarnessProfile(raw: string): { profile: HarnessProfile | null; error?: string } {
  try {
...
export function previewFromDbRow(row: DbWorkspaceRow): WorkspaceRow {
...
export function renderSonarWorkflow(): string {
  return [
    "name: SonarCloud",
    "",
    "on:",
...
export function renderCoderabbitConfig(): string {
  return [
    "reviews:",
    "  profile: chill",
...
export function generateCodeRabbitInstallUrl(): string {
  return "https://github.com/apps/coderabbitai/installations/new"
}

export function detectStack(entries: string[]): string | null {
...
export async function readTopLevelEntries(path: string, exists: boolean, isDirectory: boolean): Promise<string[]> {
  if (!exists || !isDirectory) return []
  return (await (await import("node:fs/promises")).readdir(path)).slice(0, 20)
}

export function probeGitRepoState(path: string, exists: boolean, isDirectory: boolean): { ok: boolean; stdout: string; stderr: string } {
  if (!exists || !isDirectory) return { ok: false, stdout: "", stderr: "" }
  return runGit(["rev-parse", "--is-inside-work-tree"], path)
}

export async function ensureManagedGitignore(root: string): Promise<{ changed: boolean }> {
...
export async function writeFileIfMissing(path: string, content: string): Promise<boolean> {
...
export async function buildPathPreview(
  path: string,
  allowedRoots: string[],
  readWorkspaceConfig: (root: string) => Promise<Record<string, unknown> | null>,
): Promise<Omit<WorkspacePreview, "isRegistered">> {
...
export async function ensureGitRepo(root: string, defaultBranch: string, initGit: (root: string, opts: { defaultBranch?: string; initialCommit?: boolean }) => Promise<{ ok: boolean; actions: string[]; detail?: string }>): Promise<{ ok: boolean; actions: string[]; detail?: string }> {
  ...
  if (!head.ok) {
    ...
    if (!commit.ok) return { ok: false, actions, detail: commit.stderr || "git commit failed" }
