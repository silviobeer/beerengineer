...
import { extractTarball, installStagedRelease, requestBuffer, runUpdateCheck, stageReleaseDir, stageReleaseDryRun, validateExtractedRelease, writeTarball, } from "./release.js";
...
export async function runUpdateDryRun(repos, config, opts = {}) {
    ...
    const fail = (name, err) => {
        ...
        if (prepared)
            rmSync(prepared.rootDir, { recursive: true, force: true });
    ...
    try {
        ...
        if (lock.reclaimed)
            warnings.push("stale-update-lock-reclaimed");
        stages.push({
            name: "lock",
            status: "pass",
        ...
        return fail(stages.at(-1)?.name ?? "preflight", err);
    }
}
export async function prepareUpdateApply(repos, config, opts = {}) {
    const status = buildUpdateStatus(repos, config);
    const operationId = randomUUID();
    assertUpdateSafety(status, opts.allowLegacyDbShadow);
    const check = await runUpdateCheck(config, { bypassCache: true, version: opts.version });
...
function writePreparedSwitcher(install, input) {
