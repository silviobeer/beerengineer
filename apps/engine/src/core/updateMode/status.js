...
export function buildUpdateStatus(repos, config, opts = {}) {
...
export function assertUpdateSafety(status, allowLegacyDbShadow = false) {
    if (!status.preflight.idle)
        throw new Error("update_preflight_failed:active_runs");
    if (status.preflight.lockHeld && !status.preflight.lockStale)
        throw new Error("update_preflight_failed:lock_held");
