...
export function createDatabaseBackup(config, opts) {
    ...
    for (const name of ["beerengineer.sqlite", "beerengineer.sqlite-wal", "beerengineer.sqlite-shm"]) {
        ...
        if (name !== "beerengineer.sqlite")
            src = `${opts.db.path}${name.slice("beerengineer.sqlite".length)}`;
        if (!existsSync(src))
            continue;
...
export function readLatestBackup(backupRoot) {
    return listDatabaseBackups(backupRoot, 1)[0] ?? null;
}
export function listBackupHistory(config, limit = 20) {
    return listDatabaseBackups(resolveManagedInstallPaths(config).backupRoot, limit);
}
function backupManifestPath(dir) {
    return join(dir, "manifest.json");
}
function readBackupManifest(dir) {
    const parsed = safeReadJson(backupManifestPath(dir));
    if (!parsed)
        return null;
    return { ...parsed, backupDir: dir };
}
function listDatabaseBackups(backupRoot, limit = 20) {
...
function pruneBackupRetention(backupRoot, currentBackupDir) {
    if (!existsSync(backupRoot))
        return;
    const backups = readdirSync(backupRoot, { withFileTypes: true })
        .filter(entry => entry.isDirectory())
