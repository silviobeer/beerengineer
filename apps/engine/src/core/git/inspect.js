...
import { currentBranch, itemRoot, runGit } from "./shared.js";
export function inspectWorkspaceState(workspaceRoot) {
    ...
    const branch = (branchLine.replace(/^##\s+/, "").split("...")[0] ?? "unknown").split(/\s+\[/)[0].trim();
...
export function detectGitMode(context) {
    ...
    switch (inspection.kind) {
        ...
        case "dirty":
            throw new Error(`git: workspace ${workspaceRoot} has uncommitted changes (dirty repo); commit or stash before starting`);
        case "ok":
            return {
                enabled: true,
                workspaceRoot,
...
export function assertWorkspaceRootOnBaseBranch(mode, when) {
    if (resolve(mode.workspaceRoot) === resolve(mode.itemWorktreeRoot)) {
        throw new Error(`branch_gate: workspaceRoot and itemWorktreeRoot must differ (${mode.workspaceRoot}) — ${when}`);
    }
    const actual = currentBranch(mode.workspaceRoot);
    if (actual !== mode.baseBranch) {
        throw new Error(`branch_gate: primary workspaceRoot was hijacked off ${mode.baseBranch} (now on ${actual || "<detached>"}) — ${when}`);
    }
}
export function assertActiveBranch(mode, expected, reason) {
