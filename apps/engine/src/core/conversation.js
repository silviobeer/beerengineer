 ...
 */
export function recordAnswer(repos, input) {
    const answer = input.answer.trim();
    if (!answer)
        return { ok: false, code: "empty_answer" };
    if (!repos.getRun(input.runId))
...
export function recordUserMessage(repos, input) {
...
function actorFromChat(source, role) {
    if (role === "user")
        return "user";
    if (source === "stage-agent" || source === "reviewer")
        return "agent";
    return "system";
}
/**
 ...
 */
export function buildConversation(repos, runId) {
    ...
    const entries = logs.flatMap(row => {
        if (foldedPrompts.suppressedLogIds.has(row.id))
            return [];
...
function listConversationLogs(repos, runId) {
    return repos
        .listLogsForRun(runId)
        .filter(log => log.event_type === "chat_message" ||
        log.event_type === "prompt_requested" ||
        log.event_type === "prompt_answered");
}
function buildStageKeyResolver(repos, runId) {
    const stageKeyByRunId = new Map();
    for (const sr of repos.listStageRunsForRun(runId))
        stageKeyByRunId.set(sr.id, sr.stage_key);
    return (row) => row.stage_run_id ? stageKeyByRunId.get(row.stage_run_id) ?? null : null;
}
function foldPromptMessages(logs) {
    const suppressedLogIds = new Set();
    const foldedTextByLogId = new Map();
    for (const [index, row] of logs.entries()) {
        ...
        if (!isPlaceholder && previousText !== promptText)
            continue;
        suppressedLogIds.add(previousMessage.id);
        if (isPlaceholder)
            foldedTextByLogId.set(row.id, previousMessage.message);
    }
    return { suppressedLogIds, foldedTextByLogId };
}
function findFoldablePreviousChatMessage(logs, startIndex) {
...
function conversationEntryFromLog(runId, row, stageKey, foldedTextByLogId) {
    const createdAt = new Date(row.created_at).toISOString();
    if (row.event_type === "chat_message") {
        const text = row.message.trim();
        if (!text)
            return null;
        const data = parseLogData(row.data_json);
        return {
            id: row.id,
            runId,
            stageKey,
...
function findOpenPrompt(entries) {
    const answered = new Set();
    for (const entry of entries) {
        if (entry.kind === "answer" && entry.answerTo)
            answered.add(entry.answerTo);
    }
    for (let i = entries.length - 1; i >= 0; i--) {
        const entry = entries[i];
        if (entry.kind !== "question" || !entry.promptId || answered.has(entry.promptId))
            continue;
        return {
