...
- `POST /runs/:id/recovery/skip-current-stage`
  ...
  - Response: `{ ok, runId, status, recoveryStatus }`.
  - Marks the current stage as skipped when the run has an eligible active current stage and leaves the run paused/blocked for explicit operator follow-up. The engine does not auto-advance into the next stage as part of this action.
  - `409 { error: "skip_current_stage_not_allowed", code: "skip_current_stage_not_allowed", message, reason }` when the run has no current stage, the current stage is already terminal or already skipped, or a live worker lease still owns the stage. `reason` is machine-readable and currently one of `no_current_stage | current_stage_terminal | current_stage_skipped | current_stage_actively_worked`.
- `POST /runs/:id/resume`
