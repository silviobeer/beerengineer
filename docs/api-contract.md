...
- `POST /runs/:id/recovery`
  - Canonical recovery mutation surface for both recovery action families.
  - Setup scaffolding reserves named actions on this route and currently implements only the narrow clear actions: `clear_recovery_payload`, `clear_supabase_branch_ref`, and `clear_supabase_branch_lifecycle_state`.
